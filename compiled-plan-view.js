...
(function (root) {
  ...
  const stateEffects = { RELATIONSHIP: '关系变化', KNOWLEDGE: '知识获得', RESOURCE: '资源变化', ACTIVE_CONFLICT: '冲突变化', OPEN_LOOP: '未决线索', MILESTONE_PATH: '阶段推进', ENDING_EVIDENCE: '结局证据' };
  ...
  function renderTrack(result, trackCode) {
    ...
    return `<div class="compiled-plan"><p class="experience-muted">完整冻结 Plan · ${arr(plan.narrativeSeeds).length} 个剧情种子 · ${arr(plan.agencyGraph.actors).length} 个角色 · ${arr(spine.endingProfiles).length} 类候选结局<br>以下是可达路线与触发约束，不代表本次已发生。</p>
      <details class="plan-block" open><summary>体验主线 · Experience Spine</summary>
        ...
        ${spine.chapterPlan ? `<h4>章节规划 · Rolling Chapter Window</h4><p class="experience-muted">目标约 ${esc(spine.chapterPlan.targetChapterCount)} 章；编译期只冻结当前与近未来 ${arr(spine.chapterPlan.initialWindow).length} 章，后续章节由 Runtime 根据真实 World State 动态重规划。</p>${arr(spine.chapterPlan.initialWindow).map((chapter) => `<article class="plan-entry"><strong>Chapter ${esc(chapter.ordinal)} · ${esc(chapter.title)}</strong>${facts([['叙事目标', chapter.narrativeObjective], ['结构完成条件', arr(chapter.requiredStateEffectCodes).map((code) => stateEffects[code] || code)], ['最少有效 Outcome', chapter.minimumMeaningfulOutcomes], ['聚焦剧情种子', chapter.focusSeedRefs], ['完成后的结构性结果', chapter.resolution], ['留给下一章的问题', chapter.nextQuestion], ['来源', chapter.origin]])}</article>`).join('')}` : '<p class="experience-warning">该 Plan 未返回 Chapter Plan，无法评测宏观剧情推进。</p>'}
