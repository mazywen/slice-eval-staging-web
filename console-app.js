...
(() => {
  ...
  function renderPlay() {
    ...
    return heading+'<div class="context-bar">'+V.avatar(result.input?.title,true)+'<div><h2>'+e(result.input?.title)+'</h2><span class="mono">'+e(run.runId)+'</span></div>'+V.badge(result.runtimePhase)+'<button class="button" id="add-cast-button" type="button"'+disable(!canAct())+'>添加人物</button><button class="button" id="inspect-latest" type="button">查看本次过程</button></div>'+
      ...
      '<div class="play-layout"><section class="panel play-content"><div id="play-tabs" class="tabs" role="tablist">'+Object.entries({feed:'世界动态',dm:'私聊',events:'事件',activities:'活动',cast:'人物',chapter:'章节'}).map(([key,label])=>'<button type="button" role="tab" aria-selected="'+(state.playTab===key)+'" class="'+(state.playTab===key?'active':'')+'" data-play-tab="'+key+'">'+label+'</button>').join('')+'</div>'+renderSurface()+'</section><aside class="session-aside"><section class="panel"><div class="section-heading"><h3>会话消耗</h3><small>实际采集</small></div>'+V.usageHtml(V.usage(V.allCalls(result)))+'<p class="muted">包含实际编译与当前游玩调用。费用未回传时显示未知。</p></section><section class="panel"><div class="section-heading"><h3>最近操作</h3><button class="text-button" type="button" data-page="records">全部</button></div><div class="step-list">'+stepButtons(5)+'</div></section></aside></div>';
  }
  ...
  function renderEvents(p) {
    return (state.compose?.type==='event_action'?composeForm('event_action'): '')+V.projectionNotice(p.events,'事件')+
      (I(p.events).length?I(p.events).map(row=>'<article class="entity-card"><div class="section-heading"><h3>'+e(row.title || '事件')+'</h3>'+V.badge(row.status || row.state)+'</div><p>'+e(row.description || row.setup || row.body || '')+'</p><div class="choice-list">'+A(row.choices).map(choice=>'<button class="button" type="button" data-event-choice="'+e(choice.choiceId || choice.id || '')+'" data-event-id="'+e(row.eventId)+'"'+disable(!canAct())+'>'+e(choice.label || choice.text || choice)+'</button>').join('')+'</div>'+(row.freeInputAllowed?'<button class="text-button" type="button" data-event-input="'+e(row.eventId)+'"'+disable(!canAct())+'>输入我的回应</button>':'')+V.raw('事件详情与状态',row)+'</article>').join(''):V.empty('当前没有已读取的事件','事件是否生成、何时触发，可在每次操作的调度与判定中检查。'));
  }
  ...
  function renderActivities(p) {
    ...
    return '<div class="section-heading"><h3>活动与邀请</h3><button id="create-activity" class="button small" type="button"'+disable(!canAct())+'>＋ 发起活动</button></div>'+
      ...
      attempts.map(row=>'<article class="entity-card"><div class="section-heading"><h3>'+e(row.title || row.setup?.title || row.payload?.title || '活动邀请')+'</h3>'+V.badge(row.status || row.state)+'</div><p>'+e(row.setup?.sceneDescription || row.payload?.sceneDescription || row.sceneDescription || '')+'</p><div class="message-actions"><button class="button small" type="button" data-activity-enter="'+e(row.activityAttemptId)+'"'+disable(!canAct() || row.status!=='ready')+'>进入活动</button><button class="button small" type="button" data-activity-edit="'+e(row.activityAttemptId)+'"'+disable(!canAct() || !['inviting','ready'].includes(row.status) || row.creatorActorId!==playerActorId())+'>修改</button>'+(canAnswerInvitation(row)?[['ACCEPTED','接受邀请'],['REJECTED','拒绝邀请'],['IGNORED','忽略邀请']]:[]).map(([response,label])=>'<button class="text-button" type="button" data-activity-response="'+response+'" data-attempt-id="'+e(row.activityAttemptId)+'"'+disable(!canAct())+'>'+label+'</button>').join('')+'</div>'+V.raw('活动邀请详情',row)+'</article>').join('')+
      V.projectionNotice(p.activityInstances,'进行中的活动')+instances.map(row=>'<article class="entity-card"><div class="section-heading"><h3>'+e(row.currentScene?.title || row.title || row.setup?.title || '活动')+'</h3>'+V.badge(row.status)+'</div><p>'+e(row.currentScene?.sceneDescription || row.sceneDescription || '')+'</p><small class="muted">回合 '+e(row.currentTurn ?? row.turnCount ?? '未回传')+'</small><div class="message-actions"><button class="button small primary" type="button" data-activity-turn="'+e(row.activityId)+'"'+disable(!canAct() || row.status!=='active')+'>输入活动行动</button><button class="button small" type="button" data-activity-exit="'+e(row.activityId)+'"'+disable(!canAct() || row.status!=='active')+'>退出活动</button></div>'+V.raw('活动运行详情',row)+'</article>').join('')+
  ...
  function renderSurface() {
    ...
    return '<p class="notice">章节按后台规则推进与总结。这里显示当前章节、完成依据与实际结算结果。</p>'+V.section('当前章节',chapter || outcome.chapterState || outcome.gameplayEvidence?.chapter?.state)+V.section('章节总结与结算',outcome.chapterSettlement || outcome.gameplayEvidence?.chapter?.settlement)+V.section('章节调度依据',outcome.chapterDirective || outcome.gameplayEvidence?.chapter?.directive)+V.section('总体进度',p.progression?.status==='succeeded'?p.progression.value:null)+V.section('里程碑',p.milestones?.status==='succeeded'?I(p.milestones):null)+V.section('游玩历史',p.history?.status==='succeeded'?I(p.history):null);
  }
  function tabsHtml() {
  ...
  async function task(message,fn,{renderEnd=true}={}) {
    ...
    try {
      ...
      state.busy=false;state.message=needsPolling()?'本次操作已提交，后台仍在处理。正在持续读取同一任务。':'';
