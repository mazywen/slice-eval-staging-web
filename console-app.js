...
(() => {
  ...
  document.addEventListener('click',async(event)=>{
    ...
    if(button.dataset.activityActivate){await perform({type:'activity_activate',candidateId:button.dataset.activityActivate});return;}
    if(button.dataset.allocateSkill){await perform({type:'allocate_points',skillCode:button.dataset.allocateSkill,amount:1});return;}
